...
public class Services {
    ...
    public static User login(List<User> list, String username, String password) {
        for (User x: list) {
    ...
    public static Product searchProductByName(List<Product> products, String name) {
    ...
    public static int computeFinishTime(Order order) {
        int time = 0;
        for( Product x: order.getProductList())
            time +=x.getCookingTime();
        return time;
    }
    public static int computeDeliveryCost(Order order){
        int cost=0;
    ...
    public static List<Product> initialiseProducts(){
    ...
    public static List<User> initialiseManagement(){
